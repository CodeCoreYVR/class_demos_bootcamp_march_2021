...
async function renderQuestionShow(id) {
  ...
  const { title, body, author, like_count } = question
  ...
  questionShowPage.innerHTML = `
    ...
    <div>
      <button data-action="edit" data-id="${question.id}" href="#">Edit</button>
      <button data-action="delete" data-id="${question.id}" href="#">Delete</button>
    </div>
  `
...
questionShowPage.addEventListener('click',  async event => {
  event.preventDefault()
  ...
  if (action === 'edit') {
    const { id, title, body } = await Question.one(questionId)
  ...
  if (action === 'delete') {
    await Question.destroy(questionId)
...
newQuestionForm.addEventListener('submit', async event => {
  ...
  const { id } = await Question.create(newQuestionParams)
...
editQuestionForm.addEventListener('submit', async event => {
  ...
  const { id } = await Question.update(formData.get('id'), questionParams)
